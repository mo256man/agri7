...
async function showTime() {
    ...
    if (isAuto) {
        ...
        if (time == senging_time) {                                 // 定期的な光センサー取得時間ならば
            const cond = isRun && (mode=="昼" || isNightSense);      // 条件：運転中 かつ （昼間 もしくは夜でもセンシングする設定）
            if (cond) {                                             // 条件を満たすならば
                getLight(isLightTry, true);                         // 光センサーの状態を取得し、その量を覚えておく
            } else {                                                // 条件を満たさなければ
                // addMsg("運転中ではないので光センサー更新しない");       // 何もしない
            }
            senging_time = dayjs().add(sensing_interval, "minutes").format("HH:mm:30");     // 次に光センサーを取得する時刻
        } else {                                                    // 定期的な光センサー取得時間でない　つまり毎秒
            getLight(isLightTry, false);                            // 光センサーの状態を取得するのみ
        };
...
async function getLight(isTry, bool) {
    ...
    await $.ajax("/getLight", {
        type: "post",
        data: {"isTry": isTry, "isLightCnt": bool},                         // テストか本番か　および　状態得るのみかセンサー値カウントするか　を送る
    }).done(function(data) {
        const dict = JSON.parse(data);
        console.log("光センサー");
        console.log(dict);
        try {                                                               // センサー値取得できていたら
            if (bool) {                                                     // 光センサーのデータを記憶する設定ならば
                if (dict["light_cnt"]==0) {                                 // 0回目で
                    clearLightMsg();                                        // メッセージをクリアする
                }
                msg = time + "　#" + (dict["light_cnt"]+1) + "　" + dict["log"];
                addLightLog(msg);                                           // 今回の光センサーの状態
                const th = 5*sensing_count*sensing_threshold;               // 光センサー5個*測定回数*閾値
                if (dict["light_cnt"] == sensing_count-1) {                 // 指定した回数だけセンサー値を測定したら
                    addLightLog("曇りのカウント" + dict["light_sum"] + "　　しきい値" + th);
                    console.log('dict["light_sum"]' + dict["light_sum"]);
                    if (dict["light_sum"] < th) {                           // これまでの曇りカウント累計がしきい値未満ならば
                        isLED = false;                                      // 消灯にする
                        if (lastIsLED) {                                    // さっきまで点灯していたら
                            msg = "十分明るいので消灯します　";
                            const nowjs = dayjs();
                            const lightMinutes = nowjs.diff(lightOnTime, "minute");      // lightOnTimeから今までの時間（単位：分）
                            msg += ("点灯時間 " + lightMinutes + "分");
                            console.log(msg);
                            lightMinutesSum += lightMinutes;                // ログ欄に表示
                            refreshDailyLog(lightMinutesSum);               // ログファイルに書き込み
                            showDailyLog();
//                            writeLog(time + "まで" + lightMinutes + "分間点灯　累計 " + lightMinutesSum + "分", DAYLOG)
            ...
            showLights(dict["log"]);                                    // 制御盤の光センサーランプを光らせる
...
async function getTimeMode() {
    ...
    if ((mode != lastmode)) {               // 直近と時刻モードが変わっていたら
        lastmode = mode;                    // 直近のモードを現在のモードに更新する
        switch (mode) {
            case "夜":
                msg = "　夜です　";
                isForce = true;
