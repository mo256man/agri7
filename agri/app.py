...
def getLight():
    ...
    if request.method == "POST":
        ...
        is_light_cnt = request.form["isLightCnt"]       # 光センサーの状態をカウントするか状態取得するのみか

        if is_light_cnt == "true":                      # 光センサーカウントする設定ならば
            light_cnt = (light_cnt+1) % sensing_count   # 回数を1足してカウント数で割った剰余を算出する
            print(light_cnt)
            if light_cnt == 0:                          # 余りが0ならば
                light_log = ""                          # メッセージをリセット
                light_sum = 0                           # 光センサーカウントをリセット

        lights = []                                     # 光センサーの状態の初期値
        if is_try == "true":                            # トライならば
            for _ in range(5):                          # 光センサーの数（5）だけ
                lights.append(random.choice([1, 0]))    # 0と1のどちらかを追加する
        else:                                           # 本番ならば
            lights = contec.input()                     # コンテックのボードから光センサーを取得する
            print("光センサー　本番", lights)
        
        if is_light_cnt == "true":                      # 光センサーカウントする設定ならば
            light_sum += sum(lights)                    # 光センサーが1の数をプラスする

        log = ""
        for light in lights:                            # 光センサーの状態について
            log += "○" if light==1 else "−"             # 1ならば○に、0ならば−にする
        dict = {}
...
if __name__ == "__main__":
    # command = "chromium-browser --no-sandbox --kiosk 0.0.0.0:5000"
